...
namespace Duo
{
    public class DuoApi
    {
            ...
            String cannonParams, int timeout)
        {
            ...
            request.Headers.Add("User-Agent", this.user_agent);
        ...
        public static bool HasProxyServer(HttpWebRequest requestObject)
        {
            ...
            //The Proxy Object was coming up as a new instance but no addresses, to see if it is truly non-existent we also try to get the Proxy  
            if (requestObject.Proxy != null && requestObject.Proxy.GetProxy(new Uri("http://localhost:8080")) != null)
            {
                ...
                if (!(requestObject.Proxy is WebProxy webProxyInstance))
                {
